...
class StudentMap extends BaseMap{
    ...
    public function save(User $user, Student $student){
        if ($user->validate() && $student->validate() && (new
        UserMap())->save($user)) {
        ...
        private function insert(Student $student){
            ...
            gruppa_id, num_zach) VALUES($student->user_id, $student->gruppa_id, $student->num_zach)")
            == 1) {
            return true;
            }
            return false;
        ...
        private function update(Student $student){
            ...
            $student->gruppa_id WHERE user_id=".$student->user_id) ==
            1) {
            return true;
            }
            return false;
