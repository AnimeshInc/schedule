...
class SubjectMap extends BaseMap {
    ...
    private function insert(subject $subject){
        ...
        if ($this->db->exec("INSERT INTO subject(name, otdel_id,
        hours, active)" 
        . " VALUES($name, $subject->otdel_id, $subject->hours, $active)") == 1) {
    ...
    private function update(subject $subject){
        ...
        otdel_id = $subject->otdel_id,"
        . " hours = $subject->hours, active =
        $subject->active WHERE subject_id = ".$subject->subject_id) == 1) {
        return true;
        }
        return false;
    }
    public function findAll($ofset = 0, $limit = 30){
        ...
        subject.name, subject.hours, subject.active, otdel.name AS otdel"
        . " FROM subject INNER JOIN otdel ON
    ...
    public function findViewById($id = null) {
        if ($id) {
        $res = $this->db->query("SELECT subject.subject_id,
        subject.name, subject.hours, otdel.name AS otdel"
        . " FROM subject INNER JOIN otdel ON
        subject.otdel_id=otdel.otdel_id WHERE subject_id =
        $id");
