...
class ScheduleMap extends BaseMap {
    ...
    {
        $res = $this->db->query("SELECT schedule.schedule_id, 
        lesson_num.name AS lesson_num, subject.name AS subject, 
        classroom.name AS classroom,
        CONCAT(user.lastname,' ', user.firstname, ' ',
        user.patronymic) AS fio"
        . " FROM lesson_plan INNER JOIN schedule ON 
        lesson_plan.lesson_plan_id=schedule.lesson_plan_id INNER JOIN teacher "
        . " ON lesson_plan.user_id = teacher.user_id INNER JOIN user ON 
        teacher.user_id = user.user_id INNER JOIN subject "
        . " ON lesson_plan.subject_id = subject.subject_id INNER JOIN lesson_num "
        . " ON schedule.lesson_num_id = lesson_num.lesson_num_id INNER JOIN classroom "
        . " ON schedule.classroom_id = classroom.classroom_id"
        . " WHERE lesson_plan.gruppa_id = $gruppaId 
